...
img_path = r'D:\code\carmark\mark (10).jpg'
...
ret, thr = cv2.threshold(gray_ero, 0, 255, cv2.THRESH_BINARY_INV|
                         cv2.THRESH_OTSU)
...
cnt,_ = cv2.findContours(thr, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
cnt.sort(key=lambda c: cv2.contourArea(c), reverse=True)
mask = cv2.drawContours(thr.copy(), cnt, 0, 0, cv2.FILLED)
result = cv2.bitwise_xor(thr, mask)
result = cv2.drawContours(result.copy(), cnt, 0, 255, cv2.FILLED)
ero_rgb = cv2.cvtColor(ero,cv2.COLOR_BGR2RGB)
ero_hsv = cv2.cvtColor(ero, cv2.COLOR_BGR2HSV)
# mask = cv2.inRange(ero_rgb, lower, upper)
# dst = cv2.cornerHarris(gray_ero,2,3,0.04)
# #result用于标记角点，并不重要
# dst = cv2.dilate(dst,None)
# #最佳值的阈值，它可能因图像而异。
# ero[dst>0.15*dst.max()]=[0,0,255]
# print(dst>0.1*dst.max())

# corners = cv2.goodFeaturesToTrack(gray_ero,25,0.01,10)
# corners = np.int0(corners)
# for i in corners:
#     x,y = i.ravel()
#     cv2.circle(ero,(x,y),3,255,-1)
...
cv2.imshow('mask', cv2.resize(result, None, fx=1, fy=1))
cv2.imshow('color', cv2.resize(ero, None, fx=1, fy=1))
